...
var TRACE_ENTRY = 'var __njsEntryData__ = __njsTraceEntry__({file: %s, name: %s, line: %s, args: %s, ctx: __ctx|| {}});';
var TRACE_EXIT = '__njsTraceExit__({entryData: __njsEntryData__, exception: %s, line: %s, returnValue: %s, ctx: __ctx || {}});';
...
Injector.prototype.injectTracing = function(filename, code, wrapFunctions, includeArguments, wrappedFile) {
	...
	var output = falafel(code, {ranges: true, locations: true, ecmaVersion: 8}, function processASTNode(node) {
		...
		if (node.type === Syntax.CallExpression) {
			...
			let flag = ['require','routes', 'Router','router', 'cors', 'json', 'new', 'exports', 'setTimeout'].some((elem) => {
				return ~source.indexOf(elem)
			})
			if (flag) return
			if (~source.indexOf('()')) {
				newCallExpression = source.substring(0, source.length - 1) + ('__ctx)')
			}else {
				newCallExpression = source.substring(0, source.length - 1) + (', __ctx)')
			}
			node.update(newCallExpression)
		}
		...
		if (name && node.body.type === Syntax.BlockStatement) { // Not supporting arrow functions with no body
			...
			let flag = ['resolve'].some((elem) => {
				return ~funcDec.indexOf(elem)
			})
			if (!flag) {
				if (~funcDec.indexOf('(ctx')) {
					funcDec = funcDec.replace('(ctx', '(ctx, __ctx = ctx')
				}else if (~funcDec.indexOf('()')) {
					funcDec = funcDec.substring(0, funcDec.lastIndexOf(')')) + `__ctx = '')` + funcDec.substring(funcDec.lastIndexOf(')') +1)
				}else {
					funcDec = funcDec.substring(0, funcDec.lastIndexOf(')')) + `,__ctx = '')` + funcDec.substring(funcDec.lastIndexOf(')') +1)
				}
			}
